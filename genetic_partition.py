...
class GeneticPartition:
    ...
    def delete_empty_cubes(self, part, df_vic):
        new_part = part.loc[:,df_vic.index].copy()
        return new_part

    def auc_score(self, individual, X_test, y_test):
    ...
    def colonize_ind(self, ind, X_train, y_train):
        ind.colonize(X_train, y_train)
        return ind

    def prune_ind(self, ind, min_rows_in_cube):
        ind.prune_cubes(min_rows_in_cube)
        return ind

    # main function that performs training, and selects best predictor
    def train(self, X_train, y_train, pop_size, gen_size, prob_mutate = .05,
        ...
        bounds = X_train.apply(self.get_bounds, axis=0).apply(pd.Series)
    ...
    #        perc_cluster = 0
        ...
        pop_parts = gen_mut.gen_pop(X_train, bounds, pop_size, min_cubes, max_cubes,perc_cluster)
        ...
        for enum, (train_index, test_index) in enumerate(train_test_idx):
            ...
            pop = pop_parts.apply(lambda x: PartitionClassifier(x, part_norm))
            ...
            if jobs is None:
                ...
                col_part = partial(self.colonize_ind, X_train=X_tr, y_train=y_tr)
    ...
    #                pbar.updt(len(pop),i)
        ...
        for i in range(gen_size-1):
            ...
            for enum, (train_index, test_index) in enumerate(train_test_idx):
                ...
                pop_new = pop_parts.apply(lambda x: PartitionClassifier(x, part_norm))
                X_tr = X_train.loc[train_index,:]
                y_tr = y_train.loc[train_index]
                if test_index is not None:
                    X_test = X_train.loc[test_index,:]
