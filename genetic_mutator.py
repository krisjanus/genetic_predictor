...
class GeneticMutator:
    ...
    def gen_part(self, df, bounds):
        part = pd.Series()
        for col in df.columns:
            lower = bounds.loc[col,'lower'].values
            upper = bounds.loc[col,'upper'].values
            split_p = self.gen_split(df[col], lower, upper)
            part[col] = list(set([lower, split_p, upper]))
            part[col].sort()
        return part

    def gen_centre(self, df, bounds, probability=1, cur_split=np.nan):
        part = pd.Series()
        for col in df.columns:
            lower = bounds.loc[col,'lower']
            upper = bounds.loc[col,'upper']
            col_dtype = df[col].dtypes
            split_p = self.gen_split(col_dtype, lower, upper, probability, cur_split)
            part[col] = split_p
        return part

    def gen_cluster_centres(self, df):
    ...
    def gen_pop(self, X_train, bounds, pop_size, min_cubes, max_cubes, perc_cluster=0, prefix='ind'):
        pop = pd.Series()
        ...
        for i in range(pop_size):
            ...
            if random.random() >= perc_cluster:
                pop[name] = self.gen_cube_centres(X_train, bounds, min_cubes, max_cubes)
            else:
                pop[name] = self.gen_cluster_centres(X_train)
                clust_cnt = clust_cnt + 1
            pbar.updt(pop_size,i+1)
        print(clust_cnt,'cluster individuals')
        return pop

    def gen_cube_centres(self, df, bounds, min_cubes, max_cubes):
        cubes = random.randint(min_cubes,max_cubes)
        centres = pd.DataFrame(index=df.columns)
        for i in range(cubes):
            name = 'cube_' + str(i)
            centres[name] = self.gen_centre(df, bounds)
        return centres

    def gen_split(self, column_dtype, lower, upper, probability=1, cur_split=np.nan):
    ...
    def mutate(self, surv_series, df_dtypes, bounds, probability = .01, strength = .2, keep_originals=False):
            ...
            for col in surv_series[ind].index:
                ...
                if probability >= random.random():
                    ...
                    for ind_2 in axis.index:
                        ...
                        new_col[ind_2] = self.gen_split(col_dtype, lower, upper,
                                  strength, axis[ind_2])
    ...
    def breed(self, surv_series, df_scores, nr_children_limit):
    ...
    # lengths are treated
    def breed_centroid(self, surv_series, df_scores, nr_children_limit):
        breed_series = pd.Series()
        list_of_pairs = [(surv_series.index[p1], surv_series.index[p2])
                                                for p1 in range(len(surv_series))
                                                for p2 in range(p1+1,len(surv_series))]
    ...
    def new_gen(self, population, df_train, df_scores, survival_rate, alien_rate, pop_size, prob_mutate,
        ...
        surv_breed = self.breed_centroid(survivors, df_scores[:nr_surv],pop_size-nr_surv-nr_aliens)
        surv_mut, df_report = self.mutate(surv_breed, df_train.dtypes, bounds, probability=prob_mutate,
                              strength = mutate_strength, keep_originals=keep_originals)
        alien_name = 'gen_'+str(iter_nr)+'_ind_'
        aliens = self.gen_pop(df_train, bounds, nr_aliens, min_cubes, max_cubes,
